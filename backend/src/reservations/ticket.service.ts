...
export class TicketService {
    ...
    async generateTicketPdf(reservationId: string, userId: string): Promise<Buffer> {
        ...
        if (reservation.status !== ReservationStatus.Confirmed) {
            throw new ForbiddenException('Ticket is only available for confirmed reservations');
        }
        ...
        return new Promise<Buffer>((resolve, reject) => {
            ...
            doc.fontSize(8)
                .font('Helvetica')
                .fillColor('#555555')
                .text(`#${reservation.id.slice(0, 8).toUpperCase()}`, 30, 20, { width: 540, align: 'right' });
            ...
            for (let x = 30; x < 570; x += 8) {
                doc.moveTo(x, dashY).lineTo(x + 4, dashY).stroke();
            }
            ...
            // Date
            doc.fontSize(8).font('Helvetica').fillColor('#666666').text('DATE', col1, infoY);
            doc.fontSize(13).font('Helvetica-Bold').fillColor('#ffffff')
                .text(new Date(event.dateTime).toLocaleDateString('en-US', {
                    weekday: 'short',
                    month: 'short',
                    day: 'numeric',
                    year: 'numeric',
            ...
            // Time
            doc.fontSize(8).font('Helvetica').fillColor('#666666').text('TIME', col2, infoY);
            doc.fontSize(13).font('Helvetica-Bold').fillColor('#ffffff')
                .text(new Date(event.dateTime).toLocaleTimeString('en-US', {
                    hour: 'numeric',
                    minute: '2-digit',
                    hour12: true,
                }), col2, infoY + 16);
            ...
            // Location
            doc.fontSize(8).font('Helvetica').fillColor('#666666').text('VENUE', col3, infoY);
            doc.fontSize(13).font('Helvetica-Bold').fillColor('#ffffff')
            ...
            // Attendee
            doc.fontSize(8).font('Helvetica').fillColor('#666666').text('ATTENDEE', col1, row2Y);
            doc.fontSize(13).font('Helvetica-Bold').fillColor('#ffffff')
                .text(user?.email || 'Guest', col1, row2Y + 16);

            // Status
            doc.fontSize(8).font('Helvetica').fillColor('#666666').text('STATUS', col2, row2Y);
            doc.fontSize(13).font('Helvetica-Bold').fillColor('#22c55e')
                .text('✓ CONFIRMED', col2, row2Y + 16);

            // Capacity
            doc.fontSize(8).font('Helvetica').fillColor('#666666').text('CAPACITY', col3, row2Y);
            doc.fontSize(13).font('Helvetica-Bold').fillColor('#ffffff')
            ...
            for (let x = 30; x < 570; x += 6) {
                doc.moveTo(x, 282).lineTo(x + 3, 282).stroke();
            }
            ...
            doc.fontSize(7)
                .font('Helvetica')
                .fillColor('#555555')
                .text('This ticket is valid for one person only. Please present this ticket at the venue entrance.', 30, 294, {
                    width: 350,
                });
