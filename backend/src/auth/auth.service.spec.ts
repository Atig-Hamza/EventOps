...
describe('AuthService', () => {
  ...
  describe('signup', () => {
    it('should create user and return token', async () => {
      ...
      jest.spyOn(usersService, 'createUser').mockResolvedValue({ 
          id: '1', email: 'e', password: 'hashed', role: Role.Participant 
      } as any);
    ...
    it('should throw ConflictException if email exists', async () => {
      ...
      await expect(service.signup({ email: 'e', password: 'p' })).rejects.toThrow(ConflictException);
    });
  });
  ...
  describe('login', () => {
      it('should return token if credentials valid', async () => {
          ...
          const user = { id: '1', email: 'e', password: hashedPassword, role: Role.Participant };
      ...
      it('should throw Unauthorized if user not found', async () => {
        jest.spyOn(usersService, 'findOne').mockResolvedValue(null);
        await expect(service.login({ email: 'e', password: 'p' })).rejects.toThrow(UnauthorizedException);
      });
      ...
      it('should throw Unauthorized if password wrong', async () => {
        ...
        jest.spyOn(usersService, 'findOne').mockResolvedValue(user as any);
        await expect(service.login({ email: 'e', password: 'wrong' })).rejects.toThrow(UnauthorizedException);
      });
  });
});
